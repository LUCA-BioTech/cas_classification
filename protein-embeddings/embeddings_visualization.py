...
def main(args):
    ...
    fig = visualize_clusters(data, cluster_count, min_cas_length, highlight_data=highlight_data, highlight_data2=highlight_data2, cas_types=cas_types, color_gradient=color_gradient)
    if output:
...
def visualize_clusters(data, cluster_count, min_cas_length, highlight_data=None, highlight_data2=None, cas_types=None, color_gradient=False):
    ...
    if color_gradient:
        # cast name column to int
        data['sort'] = data['name'].apply(lambda x: int(x.split('_')[1]))
        data = data.sort_values(by=['sort'])
        colors = [(i+1)/len(data) for i in range(len(data))]
        fig = px.scatter(data, x='x', y='y', color=colors, hover_data=data.columns.drop(['embeddings', 'seq']), color_continuous_scale='Viridis')
    else:
        cmap = cm.get_cmap('tab20')
        colors = [cmap(i) for i in np.linspace(0, 1, len(labels))]
        colors = [f'rgb({int(c[0] * 255)}, {int(c[1] * 255)}, {int(c[2] * 255)})' for c in colors]
        fig = px.scatter(data, x='x', y='y', color='category', hover_data=data.columns.drop(['embeddings', 'seq']), color_discrete_sequence=colors)
...
def create_parser():
    ...
    parser.add_argument('--color_gradient', action='store_true', help='color gradient')
    return parser
